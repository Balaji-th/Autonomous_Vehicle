...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(os.path.join(CWD_PATH, 'indian_road.mp4'))
...
def addText(img, radius, direction, deviation, devDirection):
    ...
    if direction == 'Straight':
        cv2.putText(img, "GO Straight", (500, 700), font, 1.5, (0, 0, 200), 3, cv2.LINE_AA)
        ser.write(bytes('F\n','utf-8'))
    elif direction == 'Left Curve':
        cv2.putText(img, "Take Left", (500, 700), font, 1.5, (0, 0, 200), 3, cv2.LINE_AA)
        ser.write(bytes('G\n','utf-8'))
    elif direction == 'Right Curve':
        cv2.putText(img, "Take Right", (500, 700), font, 1.5, (0, 0, 200), 3, cv2.LINE_AA)
        ser.write(bytes('I\n','utf-8'))
...
while True:
    ...
    cv2.putText(finalImg, "Fps: "+str(int(fps)), (30,20), cv2.FONT_HERSHEY_TRIPLEX, 0.8, (100,0, 200), 2, cv2.LINE_AA)
    pre_frame = new_frame 
